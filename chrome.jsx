// chrome.jsx — Obsidian-style shell: ribbon, vault sidebar, tabs, status bar, ⌘K palette
// Exposes: Ribbon, VaultSidebar, TabBar, StatusBar, CommandPalette, RightPanel
...
// ── Ribbon ────────────────────────────────────────────────────────────────
function Ribbon({ view, setView, onCommand, onToggleSidebar }) {
  const icons = [
...
// ── Vault sidebar (file tree) ─────────────────────────────────────────────
function VaultSidebar({ view, setView, openTab, totals, illustrations }) {
      ...
      <div className="vault-foot">
        {illustrations && (
          <svg className="coastal-decor" viewBox="0 0 220 90" preserveAspectRatio="none" aria-hidden="true">
            <path d="M0 60 Q 30 45 60 60 T 120 60 T 180 60 T 240 60 V 90 H 0 Z" fill="var(--teal)" opacity=".25" />
            <path d="M0 70 Q 30 58 60 70 T 120 70 T 180 70 T 240 70 V 90 H 0 Z" fill="var(--teal)" opacity=".4" />
            <path d="M0 80 Q 30 70 60 80 T 120 80 T 180 80 T 240 80 V 90 H 0 Z" fill="var(--teal)" opacity=".6" />
            <circle cx="180" cy="30" r="10" fill="var(--gold)" />
            <path d="M40 70 l5 -18 l3 4 l4 -10 l3 8 l4 -4 l2 20 z" fill="var(--coral)" opacity=".85" />
          </svg>
        )}
        <div className="vault-foot-row">
          <span>net worth</span>
          <strong>{window.LedgerData.fmtShort(totals.netWorth)}</strong>
        </div>
      </div>
    </aside>
  );
...
// ── Tab bar ───────────────────────────────────────────────────────────────
function TabBar({ tabs, view, setView, closeTab }) {
  return (
...
// ── Status bar ────────────────────────────────────────────────────────────
function StatusBar({ totals, view, txnCount }) {
  ...
  return (
    <div className="statusbar">
      ...
      <span className="dot">•</span>
      <span>spent {window.LedgerData.fmtShort(totals.spent)} of {window.LedgerData.fmtShort(totals.budgetTotal)}</span>
      <span className="dot">•</span>
      <span className={left >= 0 ? 'good' : 'bad'}>
        {left >= 0 ? '✓ ' : '⚠︎ '}{window.LedgerData.fmtShort(Math.abs(left))} {left >= 0 ? 'left' : 'over'}
      </span>
      <div style={{ flex: 1 }} />
...
// ── Command palette ───────────────────────────────────────────────────────
function CommandPalette({ open, onClose, setView, openTab, openAddTxn, openAddBill, openAddGoal }) {
...
Object.assign(window, { Ribbon, VaultSidebar, TabBar, StatusBar, CommandPalette, VIEW_META });
