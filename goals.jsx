...
function GoalsView({ state, setState, onAddGoal }) {
  const { fmt, fmtShort } = window.LedgerData;

  const contribute = (id, amt) => {
    setState((s) => ({
      ...s,
      goals: s.goals.map((g) => g.id === id ? { ...g, saved: Math.min(g.saved + amt, g.target) } : g),
    }));
  };
  ...
  const colorFor = (g) => {
    return { gold: '#F5D060', coral: '#FF8C00', teal: '#60B8FF', navy: '#87CEEB' }[g.color] || '#F5D060';
  };
    ...
    <div className="pane">
      <div className="page-hd">
        <div>
          ...
          <h1>Horizon Goals</h1>
          <div className="lede">
            Each goal is a shoreline you're swimming toward. Drop in a contribution any time — it pulls from your savings envelope.
          </div>
        </div>
        ...
        <div className="row" style={{ alignItems: 'flex-end', gap: 24, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 240 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginTop: 8 }}>
              ...
              <div style={{ fontSize: 14, color: 'var(--ink-3)', fontFamily: 'JetBrains Mono, monospace' }}>
                / {fmtShort(totalTarget)}
              </div>
            </div>
            <div className="goal-bar-wrap" style={{ marginTop: 14, height: 12 }}>
              <div className="goal-bar-fill" style={{ width: (totalSaved/totalTarget)*100 + '%' }}></div>
            </div>
          ...
          return (
            <div key={g.id} className="goal-card">
              ...
              <div className="goal-vals">
                <div>
                  <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.22em', color: 'var(--ink-3)', textTransform: 'uppercase' }}>Saved</div>
                  <div className="saved tnum">{fmt(g.saved).replace('.00','')}</div>
                </div>
                <div className="target tnum" style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.22em', color: 'var(--ink-3)', textTransform: 'uppercase' }}>Target</div>
                  <div>{fmt(g.target).replace('.00','')}</div>
                </div>
              </div>

              <div className="goal-bar-wrap">
                <div className="goal-bar-fill" style={{ width: pct + '%', background: `linear-gradient(90deg, ${c}, ${c}aa)` }}></div>
              </div>
