...
import { fmt, today } from './data.js';
import { KPI } from './components.jsx';

export function BillsView({ state, setState, onAddBill }) {
    ...
    <div className="pane">
      <div className="page-hd">
        <div>
          ...
          <h1>Bills & Subs</h1>
          <div className="lede">Recurring tides. Every charge that hits your account on autopilot, organized by what's cresting next.</div>
        </div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '40px 1fr 100px 140px 80px 80px', gap: 14, padding: '8px 12px', fontSize: 9, fontWeight: 800, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--sl-ink-3)' }}>
          <span /><span>Name</span><span style={{ textAlign: 'right' }}>Amount</span><span>Next charge</span><span>Auto</span><span style={{ textAlign: 'right' }}>Active</span>
        </div>
        ...
        {annotated.map((b) => (
          <div key={b.id} className={'bill-row ' + (!b.active ? 'inactive' : '')}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: 'var(--sl-ink-3)', marginTop: 2 }}>
            ...
            <div>{b.auto ? <span className="tag gold">auto</span> : <span className="tag">manual</span>}</div>
            <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', alignItems: 'center' }}>
              <button className="toggle" data-on={b.active ? 1 : 0} onClick={() => toggleActive(b.id)}><i /></button>
