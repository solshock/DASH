// app.jsx — main app: state, tweaks, modals, routing
// Renders at #root
...
function App() {
  ...
  const totals = React.useMemo(() => window.LedgerData.deriveTotals(state), [state]);
  ...
  const openTab = (id) => setTabs((tabs) => tabs.includes(id) ? tabs : [...tabs, id]);
  const closeTab = (id) => {
    setTabs((tabs) => {
  ...
  // ── Modals: add txn / bill / goal ──────────────────────────────────────
  const [modal, setModal] = React.useState(null); // 'txn' | 'bill' | 'goal' | null
      ...
      <TweaksPanel title="Tweaks · Solshock Ledger">
        ...
        <TweakButton label="Reset all data" secondary onClick={() => {
          if (confirm('Reset to seed data? This will wipe your transactions and edits.')) {
            setState(JSON.parse(JSON.stringify(window.LedgerData.seed)));
          }
        }} />
  ...
  return (
    <Modal title="New bill or subscription" onClose={onClose} onSave={save} saveLabel="Add bill">
      ...
      <div className="row" style={{ gap: 10 }}>
        <button className="toggle" data-on={form.auto ? 1 : 0} onClick={() => update('auto', !form.auto)}><i></i></button>
        <span style={{ fontSize: 12, color: 'var(--ink-2)' }}>Charges automatically</span>
      </div>
    </Modal>
  );
}
          ...
          {[['gold', '#F5D060'], ['coral', '#FF8C00'], ['teal', '#60B8FF'], ['navy', '#87CEEB']].map(([k, hex]) => (
            <button
              ...
              style={{
                ...
                background: hex, border: '2px solid ' + (form.color === k ? 'var(--gold-light)' : 'transparent'),
                cursor: 'pointer'
              }}
            />
          ))}
