// debt.jsx — Debt payoff with snowball/avalanche projection
// Exposes: DebtView

function DebtView({ state, setState }) {
    ...
    <div className="pane">
      <div className="page-hd">
        <div>
          ...
          <h1>Debt Tides</h1>
          <div className="lede">
            What you owe, in retreating order. Switch between snowball (smallest balance) and avalanche (highest rate) to see your payoff horizon move.
          </div>
        </div>
        <div className="page-hd-right">
          <span className="card-sub" style={{ marginRight: 4 }}>strategy</span>
          <div className="row" style={{ gap: 4, background: 'rgba(255,255,255,0.04)', padding: 3, borderRadius: 8, border: '1px solid var(--line-2)' }}>
              ...
              <div key={d.id} className="debt-row">
                ...
                <div>
                  ...
                  <div className="debt-progress-wrap">
                    <div className="debt-progress-fill" style={{ width: pct + '%' }}></div>
                  </div>
                ...
                <div>
                  ...
                  <div className="debt-meta-val" style={{ textAlign: 'right', color: 'var(--gold-light)' }}>{d.payoffMonths}mo</div>
                </div>
              </div>
            );
          })}
        ...
        <div className="card">
          ...
          <div className="field" style={{ marginBottom: 20 }}>
            ...
            <div className="row" style={{ alignItems: 'center', gap: 12 }}>
              <input type="range" min="0" max="800" step="25" value={extra} onChange={(e) => setExtra(+e.target.value)} style={{ flex: 1, accentColor: 'var(--gold)' }} />
              <div className="env-vals" style={{ width: 70, textAlign: 'right' }}>${extra}</div>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <ProjectionStat label="Total interest paid"   value={fmt(projection.interest)} sub={`vs ${fmt(projection.interestNoExtra)} with min only`} highlight />
          ...
          <div style={{ marginTop: 20, padding: 14, background: 'rgba(96,184,255,0.06)', border: '1px solid rgba(96,184,255,0.2)', borderRadius: 8, fontSize: 12, color: 'var(--foam)', lineHeight: 1.5 }}>
            <strong style={{ color: 'var(--electric-blue)' }}>🌊 Coastal tip.</strong> {strategy === 'avalanche'
...
function ProjectionStat({ label, value, sub, highlight, gold }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--line-2)' }}>
      <div>
...
function simulate(debts, strategy, extraInput) {
  // Returns: months, interest, ordered (with payoffMonths), interestNoExtra, monthsNoExtra
  function run(extra) {
    const ds = debts.map((d) => ({ ...d, _bal: d.balance, _paid: 0 }));
    let month = 0;
    let totalInterest = 0;
    let payoffMonths = {};
    while (ds.some((d) => d._bal > 0) && month < 600) {
      ...
      const sorted = [...ds].filter((d) => d._bal > 0).sort((a, b) => {
        return strategy === 'avalanche' ? b.apr - a.apr : a._bal - b._bal;
      });
      if (sorted[0] && pool > 0) {
        const pay = Math.min(pool, sorted[0]._bal);
        sorted[0]._bal -= pay;
        sorted[0]._paid += pay;
      }
      // Record payoff
  ...
  // Build ordered list with payoffMonths
